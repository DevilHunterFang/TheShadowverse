...
public class LelouchCollaboration extends AbstractEvent {
    ...
    protected void buttonEffect(int buttonPressed) {
        ...
        switch (buttonPressed) {
            case 0:
                if (this.screenNum == 0) {
                    ...
                    if (AbstractDungeon.player.hasRelic(GeassRelic.ID)) {
                        ...
                        AbstractDungeon.getCurrRoom().addRelicToRewards(new GeassRelic());
                    }
                    enterCombat();
                }
                return;
            case 1:
                geass = new Geass();
                AbstractDungeon.effectList.add(new ShowCardAndObtainEffect(geass, Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F));
